...
async function loadSources() {
  ...
  data.sources.forEach((s) => {
    ...
    const statusClass = s.last_error ? "status-err" : (s.ingest_status === "ingesting" ? "status-ok" : "status-ok");
    const statusText = s.last_error ? `error: ${s.last_error}` : (s.ingest_status === "ingesting" ? "ingesting…" : "healthy");
    li.innerHTML = `
      <div class="row1">
