...
# Render can receive a manual ingest request at the same time that the
# background scheduler decides a source is due. ChromaDB uses a local SQLite
# store, so serialize all ingestion writes in this process.
...
def ingest_source(source: dict, force: bool = False) -> dict:
    ...
    All ingestion writes are serialized because Render runs ChromaDB on the
    local filesystem. This prevents a manual refresh and the scheduler from
    trying to write the same SQLite/vector store at the same time.
    """
    with _ingest_lock:
