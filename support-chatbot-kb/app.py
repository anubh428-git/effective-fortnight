"""
Flask web app for the self-updating support chatbot (Streamlit-free UI).

Routes:
  GET  /                         -> chat + knowledge-source management UI
  POST /api/chat                 -> {message} -> RAG answer
  GET  /api/sources              -> list configured sources + chunk counts
  POST /api/sources              -> add a source ({type, location, ...}) and ingest it immediately
  DELETE /api/sources/<id>       -> remove a source and its vectors
  POST /api/sources/<id>/refresh -> force re-ingest a single source now
  POST /api/sources/refresh-all  -> force re-ingest every source now
"""
import time
import uuid
import threading

from flask import Flask, request, jsonify, render_template

import config
from scheduler import KBScheduler, ingest_source, run_all_due
from chatbot import answer as chatbot_answer
from knowledge_base import KnowledgeBase
import language
import translation

app = Flask(__name__)
kb = KnowledgeBase()
kb_scheduler = KBScheduler(poll_seconds=60)
kb_scheduler.start()  # started at import time so it also runs under gunicorn,
                       # not just `python app.py`


@app.route("/")
def index():
    return render_template("index.html")


@app.route("/api/chat", methods=["POST"])
def chat():
    data = request.get_json(force=True) or {}
    question = (data.get("message") or "").strip()
    session_id = data.get("session_id")
    if not question:
        return jsonify({"error": "message is required"}), 400
    result = chatbot_answer(question, session_id=session_id)
    # hits carry raw chunk text for internal use; trim before returning
    result.pop("hits", None)
    return jsonify(result)


@app.route("/api/languages", methods=["GET"])
def languages():
    return jsonify(
        {
            "supported": language.SUPPORTED_LANGUAGES,
            "pivot_language": language.PIVOT_LANGUAGE,
            "translation_backend": translation.backend_name(),
        }
    )


@app.route("/api/sources", methods=["GET"])
def list_sources():
    sources = config.load_sources()
    for s in sources:
        s["chunk_count"] = kb.get_source_chunk_count(s["id"])
    return jsonify({"sources": sources, "stats": kb.stats()})


@app.route("/api/sources", methods=["POST"])
def add_source():
    data = request.get_json(force=True) or {}
    s_type = data.get("type")
    location = data.get("location")
    if s_type not in ("url", "text", "file") or not location:
        return jsonify({"error": "type (url|text|file) and location are required"}), 400

    source = {
        "id": data.get("id") or f"{s_type}-{uuid.uuid4().hex[:8]}",
        "type": s_type,
        "location": location,
        "update_interval_minutes": int(data.get("update_interval_minutes", 60)),
        "created_at": time.time(),
        "last_checked_at": 0,
        "last_updated_at": None,
        "last_content_hash": None,
        "last_error": None,
        "ingest_status": "ingesting",
    }
    config.upsert_source(source)

    # Do not keep the browser/Render request open while downloading a page and
    # writing ChromaDB. Render's proxy can time out long synchronous requests.
    # The UI already polls /api/sources every 15 seconds, so it can observe the
    # result when the background job finishes.
    def _background_ingest():
        ingest_source(source, force=True)

    threading.Thread(
        target=_background_ingest,
        name=f"ingest-{source['id']}",
        daemon=True,
    ).start()

    return jsonify({"source": source, "ingest_result": {"status": "started"}}), 202


@app.route("/api/sources/<source_id>", methods=["DELETE"])
def delete_source(source_id):
    kb.delete_source(source_id)
    config.remove_source(source_id)
    return jsonify({"status": "deleted", "id": source_id})


@app.route("/api/sources/<source_id>/refresh", methods=["POST"])
def refresh_source(source_id):
    source = config.get_source(source_id)
    if not source:
        return jsonify({"error": "not found"}), 404
    source["ingest_status"] = "ingesting"
    source["last_error"] = None
    config.upsert_source(source)
    threading.Thread(
        target=lambda: ingest_source(source, force=True),
        name=f"refresh-{source_id}",
        daemon=True,
    ).start()
    return jsonify({"id": source_id, "status": "started"}), 202


@app.route("/api/sources/refresh-all", methods=["POST"])
def refresh_all():
    # Start one background job so the browser request returns immediately.
    threading.Thread(
        target=lambda: run_all_due(force=True),
        name="refresh-all",
        daemon=True,
    ).start()
    return jsonify({"status": "started"}), 202


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=False)
